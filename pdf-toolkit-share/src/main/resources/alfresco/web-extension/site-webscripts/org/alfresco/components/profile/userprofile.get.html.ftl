...
<script type="text/javascript">//<![CDATA[
   var userProfile = new SomeCo.UserProfile("${args.htmlid}").setOptions(
   {
      ...
      profile: {
         ...
         signaturePin: "<#if profile.properties.signaturePin??>${profile.properties.signaturePin?js_string}</#if>"
         }
   }).setMessages(
      ${messages}
   );
   ...
   <@uniqueIdDiv>
      ...
      <div id="${el}-body" class="profile">
         <div id="${el}-readview" class="hidden">
            ...
            <@markup id="viewSomeCoInfo">
            	<div class="header-bar">${msg("label.somecoInfo")}</div>
		         ...
		         <#if profile.properties.signaturePin?? && profile.properties.signaturePin?length!=0>
		         <div class="row">
         ...
         <div id="${el}-editview" class="hidden">
		    ...
		    <div class="row">
		         <span class="label"><label for="${el}-input-signaturePin">${msg("label.signaturePin")}:</label></span>
		         <span><input type="password" maxlength="256" size="30" id="${el}-input-signaturePin" value="" <@immutablefield field="signaturePin" /> /></span>
		    </div>
            </@markup>
            <hr/>
